    ...
    def add_knowledge(self, cell, count):
        ...
        for x in range(-1, 2):
            for y in range(-1, 2):
                ...
                nei_cell = (i + x, j + y)
                if nei_cell in self.safes:
                    continue
                if nei_cell in self.mines:
                    sentence.count -= 1
                    continue
                sentence.cells.add(nei_cell)
        self.knowledge.append(sentence)

        # Recursively check for new information
        def check_knowledge():
            # Check if any sentence is all mines or all safes
            for sentence in self.knowledge:
            ...
            # Check if inference is possible berween any two sentences
            for sentence1 in self.knowledge:
                for sentence2 in self.knowledge:
                    if sentence1.cells <= sentence2.cells and not sentence2.cells <= sentence1.cells:
